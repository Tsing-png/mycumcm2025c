...
问题一和问题二仅纳入孕周与BMI两个因素。问题三考察年龄和体重对Y染色体浓度预测的增量贡献，并在多因素框架下重新优化分组检测时点。体重、身高与BMI之间存在严重的多重共线性，直接纳入会导致参数估计不稳定，需要先进行变量解耦。
...
为保留体重中独立于BMI和身高的信息，采用残差分解法。将体重对BMI和身高进行辅助回归
...
分解后所有VIF均接近1，共线性完全消除。图~\ref{fig:vif_comparison}直观展示了分解前后VIF的对比。
...
两个变量的$p$值均远大于0.05，未达到统计显著性。BMI的非线性样条效应在M1中仍为最主要的预测成分，有效自由度和$F$统计量与问题一模型基本一致。

换言之，在已纳入BMI非线性效应的条件下，年龄和体重（扣除BMI和身高的线性贡献后）对Y染色体浓度不具备额外预测能力。BMI已充分捕获了体脂水平对胎儿游离DNA稀释效应的影响。

\subsection{达标比例与风险优化}

\subsubsection{风险函数构建}

问题二的最优时点仅基于群体预测均值确定，未考虑个体间预测分布的离散程度。为更全面地权衡检测时机，定义达标比例（compliance rate）为BMI分组$g$在孕周$t$时，预测Y染色体浓度不低于检测阈值$c=0.04$的个体占比
...
延迟检测风险$R_{\text{late}}(t)$定义为分段线性函数
...
该函数反映检测时点越晚，后续干预窗口越窄所带来的风险递增。综合两项指标，构建加权风险函数
...
前三组的最优时点集中在12.8至13.3周，与问题二的结论基本吻合。BMI$\geq$40组的最优时点被推迟至24.8周，该组胎儿游离DNA浓度增长极为缓慢，在12至20周区间内达标比例始终偏低，风险函数直到接近观测窗口上限才取得最小值。图~\ref{fig:compliance_curves}展示了各组达标比例随孕周的变化趋势。
...
上述最优时点依赖于GAMM的点估计和固定的残差方差$\sigma^2_\varepsilon$。测量噪声的实际水平存在不确定性，可能改变达标比例的计算进而影响最优时点。本文采用Monte Carlo误差传播方法，在不同噪声水平下重复优化，获得最优时点的经验分布。

具体步骤如下。

\textbf{Step 1}（噪声扰动）\quad 设模型估计的残差标准差为$\hat{\sigma}_\varepsilon$。在每次Monte Carlo迭代中，从均匀分布$U(0.5,\,1.5)$中抽取缩放因子$\lambda$，将残差标准差设为$\lambda \hat{\sigma}_\varepsilon$，模拟测量精度在$0.5$倍至$1.5$倍范围内波动。
...
前两组（BMI $<$ 36）的最优时点分布集中，标准差分别为0.64周和0.81周，95\%置信区间宽度不超过3.1周，时点推荐稳健可靠。BMI在$[36, 40)$组的标准差增至3.04周，置信区间跨度达12.3周，推荐精度下降明显。BMI$\geq$40组的置信区间从11.0周延伸至24.6周，跨度达13.6周，最优时点在噪声扰动下几乎不可确定。该组仅4名孕妇，样本量严重不足是估计不稳定的根本原因。

\subsection{实际意义}

问题三的分析提供了三项定量依据。

第一，年龄和体重（在控制BMI之后）对Y染色体浓度不具备额外预测能力。临床采集BMI即可充分刻画母体因素对胎儿游离DNA稀释效应的影响，无需额外建模其他人体测量指标，可简化检测前的信息采集流程。

第二，BMI低于36的孕妇群体（占样本的92.5\%），最优检测时点稳定在12.8至13.3周，Monte Carlo分析确认该推荐在测量噪声波动下可靠。

第三，BMI$\geq$36尤其是BMI$\geq$40的高BMI群体，最优时点的不确定性极大，当前样本量不足以支撑可靠的时点推荐。对这部分人群，建议在扩大样本的前提下重新评估，临床决策中对单一模型输出保持审慎。
