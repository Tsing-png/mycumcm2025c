...
NIPT通过检测孕妇外周血中的胎儿游离DNA来筛查染色体异常，其准确性依赖于胎儿DNA浓度是否达到检测阈值。男胎Y染色体浓度随孕周增长而上升，但受孕妇BMI的稀释效应影响，不同体型孕妇达标时间差异较大。检测过早则浓度不足、结果不可靠，检测过晚则错过治疗窗口、风险升高。本文围绕如何为不同BMI孕妇确定最佳NIPT时点，以及如何判定女胎染色体异常，建立了四个层层递进的数学模型。

针对问题一，首先，数据为267名孕妇共1082条纵向观测，同一孕妇多次测量的组内相关系数ICC达0.60，普通回归会严重低估标准误。其次，孕周与浓度的关系可能非线性，线性假设过于粗糙。然后，建立\textbf{广义加性混合模型（GAMM）}，用B样条基函数拟合孕周和BMI的非线性效应，随机截距与随机斜率刻画个体差异。最后，GAMM的AIC为\textbf{$-$5240.80}，较线性混合效应模型（AIC=$-$5174.04）改善显著，似然比检验卡方值为\textbf{76.76}，孕周呈非线性上升趋势，BMI对浓度有负向抑制作用。

针对问题二，首先，不同BMI孕妇的Y染色体浓度达到4\%阈值的时间差异明显，统一检测时点无法兼顾准确性与时效性。其次，构造双风险函数，将延迟发现风险与浓度不足导致的检测不准确风险加权整合为可优化的目标。然后，以BIC准则在候选断点网格上搜索最优BMI分组，得到断点30和36，将孕妇分为三组。最后，优化得到各组最佳NIPT时点分别为\textbf{11.7周}、\textbf{11.8周}、\textbf{13.5周}，高BMI组对检测误差最敏感，阈值偏移2\%可将其最优时点推迟约8周。

针对问题三，首先，题目要求纳入身高、体重、年龄等多因素，但BMI与体重的方差膨胀因子VIF高达351，直接纳入会导致多重共线性。其次，采用残差分解，提取体重偏离BMI预期的残差作为独立变量，VIF降至\textbf{1.01}。然后，扩展GAMM纳入年龄和体重残差，并用500次Monte Carlo模拟量化测量误差对时点估计的传播效应。最后，年龄和体重残差均不显著，BMI单因素已足够。细分4组后最优时点为12.8、13.3、13.3、24.8周，高BMI组95\%置信区间宽达[11.0, 24.6]周，样本量不足是主要原因。

针对问题四，首先，女胎不携带Y染色体，无法用浓度达标判断检测准确性，且数据中女胎全部标记为健康，缺乏监督学习所需的正样本。其次，选择无监督的\textbf{多元马氏距离异常检测}，利用13、18、21号及X染色体Z值、BMI、X浓度共6维特征的协方差结构识别偏离正常模式的样本。然后，以非整倍体记录作为评估标准，对比马氏距离法与单变量Z值阈值法。最后，马氏距离法AUC为\textbf{0.574}，优于Z值阈值法的0.492，最佳F1为0.217，13号染色体异常最易检出，21号染色体异常检出困难。

本文创新点包括（1）GAMM捕捉孕周非线性效应并以似然比检验量化改进幅度（卡方值76.76）；（2）双风险函数将NIPT时点选择转化为可优化的决策问题，延迟风险与准确性风险同时进入目标函数；（3）残差分解消除体重与BMI共线性（VIF从351降至1.01），Monte Carlo模拟量化高BMI组时点不确定性。
