...
NIPT通过检测孕妇外周血中胎儿游离DNA筛查染色体异常。男胎Y染色体浓度随孕周上升但受BMI稀释，检测过早浓度不足、过晚则错过治疗窗口。本文建立四个递进模型解决时点选择与异常判定问题。

针对问题一，首先，267名孕妇1082条纵向数据组内相关系数达0.60。其次，孕周与浓度存在非线性关系。然后，建立\textbf{广义加性混合模型（GAMM）}，B样条拟合非线性，随机效应处理个体差异。最后，AIC为\textbf{$-$5240.80}，似然比卡方值\textbf{76.76}，非线性改进极显著。

针对问题二，首先，不同BMI孕妇浓度达标时间差异明显。其次，构造双风险函数加权延迟风险与不准确风险。然后，BIC准则搜索得BMI断点30和36。最后，三组最佳时点\textbf{11.7}、\textbf{11.8}、\textbf{13.5}周，高BMI组对误差最敏感。

针对问题三，首先，BMI与体重VIF达351。其次，残差分解降至\textbf{1.01}。然后，扩展GAMM并500次MC模拟量化误差传播。最后，年龄和体重残差不显著，BMI单因素已足够。

针对问题四，首先，女胎数据缺乏正样本。其次，\textbf{多元马氏距离异常检测}利用6维特征协方差结构。最后，AUC为\textbf{0.574}，优于Z值阈值法0.492。

创新点（1）GAMM捕捉非线性并量化改进；（2）双风险函数转化时点为优化问题；（3）残差分解消除共线性，MC量化不确定性。

\keywords{NIPT检测；广义加性混合模型；BMI分段优化；Monte Carlo误差传播；马氏距离异常检测}
