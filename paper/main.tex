...
针对问题一，首先，数据为267名孕妇共1082条纵向观测，同一孕妇多次测量的组内相关系数ICC达0.60，普通回归会严重低估标准误。其次，孕周与浓度的关系可能非线性，线性假设过于粗糙。然后，建立\textbf{广义加性混合模型（GAMM）}，用B样条基函数拟合孕周和BMI的非线性效应，随机截距与随机斜率刻画个体差异。最后，GAMM的AIC为\textbf{$-$5240.80}，较线性混合效应模型改善显著，似然比检验卡方值为\textbf{76.76}，孕周呈非线性上升趋势，BMI对浓度有负向抑制作用。
...
创新点包括（1）GAMM捕捉孕周非线性效应并以似然比检验量化改进幅度（卡方值76.76）；（2）双风险函数将NIPT时点选择转化为可优化的决策问题，延迟风险与准确性风险同时进入目标函数；（3）残差分解消除体重与BMI共线性（VIF从351降至1.01），Monte Carlo模拟量化高BMI组时点不确定性。
