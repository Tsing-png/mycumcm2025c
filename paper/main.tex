...
针对问题一，首先，267名孕妇1082条纵向数据中同一孕妇多次测量的组内相关系数达0.60，普通回归会低估标准误。其次，孕周与浓度的关系存在非线性特征，线性假设拟合不足。然后，建立\textbf{广义加性混合模型（GAMM）}，用B样条拟合非线性效应，随机截距与随机斜率处理个体差异。最后，GAMM的AIC为\textbf{$-$5240.80}，较线性混合效应模型改善显著，似然比检验卡方值\textbf{76.76}，Y染色体浓度随孕周非线性上升，BMI有负向抑制作用。

针对问题二，首先，不同BMI孕妇浓度达到4\%的时间差异明显，统一时点无法兼顾准确性与时效性。其次，构造双风险函数，将延迟风险与检测不准确风险加权为可优化目标。然后，以BIC准则搜索最优BMI分组断点，得到断点30和36，分为三组。最后，各组最佳NIPT时点为\textbf{11.7周}、\textbf{11.8周}、\textbf{13.5周}，高BMI组对误差最敏感，阈值偏移2\%可使时点推迟约8周。

在问题二模型基础上，针对问题三，首先，BMI与体重的方差膨胀因子高达351，直接纳入多因素会产生严重共线性。其次，采用残差分解提取体重偏离BMI预期的残差，VIF降至\textbf{1.01}。然后，扩展GAMM纳入年龄和体重残差，500次Monte Carlo模拟量化误差传播。最后，年龄和体重残差均不显著，BMI单因素已足够。细分4组后高BMI组95\%置信区间宽达[11.0, 24.6]周，样本量不足是主要原因。

针对问题四，首先，女胎不携带Y染色体，且数据中全部标记为健康，缺乏监督学习所需正样本。其次，选择无监督的\textbf{多元马氏距离异常检测}，利用6维特征的协方差结构识别异常。然后，以非整倍体记录为评估标准对比两种方法。最后，马氏距离法AUC为\textbf{0.574}，优于Z值阈值法的0.492，13号染色体异常最易检出，21号检出困难。

本文创新点包括（1）GAMM捕捉孕周非线性效应，似然比检验量化改进幅度；（2）双风险函数将NIPT时点选择转化为可优化决策问题；（3）残差分解消除共线性（VIF从351降至1.01），Monte Carlo模拟量化高BMI组时点不确定性。
