...
G1与G2组的最优时点十分接近（约11.7$\sim$11.8周），最小风险均低于0.025，表明中低BMI孕妇在孕12周前即可获得高置信度的检测结果。G3组（BMI$\geq$36）的最优时点推迟至13.49周，最小风险升高至0.0675，约为前两组的3倍。这一差异源于高BMI孕妇的母体游离DNA对胎儿DNA的强稀释效应，使浓度达标时间整体后移。图~\ref{fig:q2_conc}展示了三组孕妇的浓度预测曲线，高BMI组浓度增长更缓慢。
...
两个变量的$p$值均远大于0.05，未达到统计显著性。BMI的非线性样条效应在M1中仍为最主要的预测成分。这一结果表明，在已纳入BMI非线性效应的条件下，年龄和体重（扣除BMI和身高的线性贡献后）对Y染色体浓度不具有显著的额外预测能力。图~\ref{fig:q3_diag}给出了多因素GAMM的残差诊断结果。
...
前三组的最优时点集中在12.8至13.3周，与问题二的结论基本吻合。BMI$\geq$40组的最优时点被推迟至24.8周，原因在于该组胎儿游离DNA浓度增长极为缓慢，在12至20周区间内达标比例始终偏低，风险函数直到接近观测窗口上限才取得最小值。各组达标比例随孕周的变化如图~\ref{fig:compliance_curves}所示。
...
上述最优时点的确定依赖于GAMM的点估计和固定的残差方差$\sigma^2_\varepsilon$。为量化测量噪声不确定性对结果的影响，本文采用Monte Carlo误差传播方法。在每次迭代中，从均匀分布$U(0.5,\,1.5)$中抽取缩放因子$\lambda$，将残差标准差设为$\lambda \hat{\sigma}_\varepsilon$，重新执行达标比例模拟并搜索最优孕周。重复500次，收集每组最优时点的经验分布。结果如表~\ref{tab:q3_mc}所示，各组时点的经验分布见图~\ref{fig:mc_distributions}。
...
M1的AUC-ROC较M2高出0.082，在精确率-召回率权衡上也更优。M2虽然召回率较高，但精确率仅为10.6\%，大量正常样本被误判为异常，实际可用性不足。图~\ref{fig:q4-roc}对比了M1在不同配置下的ROC曲线，图~\ref{fig:q4-dist}展示了正常与异常样本的马氏距离分布，图~\ref{fig:q4-zdist}给出了两类样本在各染色体Z值上的分布对比。
...
文件名 & 说明 \\
\midrule
code/Q1/q1\_main.py & 问题一GAMM与LME建模主程序 \\
code/Q1/q1\_robustness.py & 问题一鲁棒性分析（样条自由度、交叉验证） \\
code/Q2/q2\_main.py & 问题二BMI分组与最优NIPT时点优化主程序 \\
code/Q2/q2\_robustness.py & 问题二鲁棒性分析（断点、权重、阈值敏感性） \\
code/Q3/q3\_main.py & 问题三多因素GAMM与Monte Carlo误差传播主程序 \\
code/Q3/q3\_robustness.py & 问题三鲁棒性分析（MC收敛、分解稳定性） \\
code/Q4/q4\_main.py & 问题四马氏距离异常检测主程序 \\
code/Q4/q4\_baseline.py & 问题四Z值阈值基线方法 \\
code/Q4/q4\_robustness.py & 问题四鲁棒性分析（特征子集、阈值、留一验证） \\
code/Q4/run\_all.py & 问题四M1与M2对比运行脚本 \\
workspace/data\_raw/附件.xlsx & 原始数据（题目附件） \\
\bottomrule
\end{tabular}
\end{table}

以上所有Python程序均可独立运行，随机种子统一设为42，保证结果可复现。
