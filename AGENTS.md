...
- 摘要：限制一页篇幅。每个问题摘要统一以针对问题 X：开头；问题描述使用首先、其次、然后、最后串联行文。摘要末尾罗列模型创新点，创新点带有序号，序号不必强制换行。在说用了什么方法前，可以适当说明题目特点，给方法选择提供依据。
- 每个问题段落内需要写明加粗展示模型名称，同时展示实验结果，关键数值加粗标注。摘要总字数控制 800–900 字，正文常规字体。
